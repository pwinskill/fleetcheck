...
## Seconds per scenario for falciparum, run here. A vivax run costs over a second
## per simulated year -- most of a minute a scenario -- so under vivax fleet's
## runs join the IBM's on the worker pool below instead of queueing one after
## another.
FLEET_ON_POOL <- SP == "pv"
...
ran <- parallel::parLapplyLB(cl, seq_len(nrow(jobs)), function(j) {
  ...
  if (jobs$model[j] == "fleet") {
    ## exactly as run_fleet() does it: default tuning, every row
    el <- system.time(out <- fleet::run_simulation_ode(
      timesteps = s$years * 365, parameters = s$p))[["elapsed"]]
  } else {
    set.seed(1000L + 7L * k)
