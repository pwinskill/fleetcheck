...
## ---- 4c. country site files (a SNAPSHOT) -------------------------------------------
## As for falciparum (tables.R, section 4): read from a committed snapshot, because
## the sweep needs the site files, which are not redistributable. Clinical only:
## the IBM has no vivax severe disease. CMP_REFRESH_SITES=1, after the vivax sweep,
## re-takes it.
...
if (nzchar(Sys.getenv("CMP_REFRESH_SITES"))) {
  ...
  if (!length(fs)) stop("CMP_REFRESH_SITES is set but there are no results in ", raw,
                        " -- run CMP_PARASITE=pv validations/03-real-settings/run.R first.")
  ...
  ## the sweep's own provenance, not this pass's (see tables.R)
  ref <- file.path(fc_results("03-real-settings", "pv"), "sites_reference.json")
  run <- if (file.exists(ref)) {
    r <- jsonlite::read_json(ref, simplifyVector = TRUE)
    list(taken = substr(r$generated, 1, 10), fleet = r$fleet, n_age_groups = r$n_age_groups)
  } else prev$run %||% list()
  snap <- list(
    run = run,
    summarised = stamp(files = length(fs)),
    note = paste(
      "The P. vivax site-file comparison: up to ten vivax sub-sites per country,",
      "spread across its vivax EIR range. It re-runs fleet only -- the IBM arm is",
      "the pre-run diagnostic shipped with each site file -- but needs the",
      "malariaverse site files, which are not redistributable. Re-take it with",
      "CMP_PARASITE=pv validations/03-real-settings/run.R and assess.R, then",
      "CMP_REFRESH_SITES=1 on render_pv.R and tables_pv.R."),
    countries = length(unique(v$iso3c)),
    sub_sites = nrow(distinct(v, iso3c, name_1, urban_rural)),
    year_from = min(v$year), year_to = max(v$year),
    clinical = as.list(agreement(v$ms_clinical, v$fleet_clinical)))
  jsonlite::write_json(snap, site_f, auto_unbox = TRUE, pretty = TRUE, digits = 6)
  message("re-took the vivax site snapshot (", snap$countries, " countries)")
}
if (file.exists(site_f)) {
  ...
  say("clinical: n = %s, r = %.3f, slope = %.3f, relative bias = %s",
