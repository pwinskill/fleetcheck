...
panel_eir <- function(metric_id, ylab) {
  ...
  ggplot() +
    ...
    geom_point(data = go, aes(init_EIR, mid, colour = model, shape = model, fill = model),
               size = 2.6, stroke = 0.5) +
    scale_x_log10(breaks = EIR_GRID_PV, labels = scales::label_number(drop0trailing = TRUE),
                  minor_breaks = NULL) +
...
for (m in c(EIR_MET, EIR_ONE)) {
  dev <- panel_deviation(filter(ibm_e, metric == m$key), filter(ode_e, metric == m$key),
                         EIR_GRID_PV)
  one <- (panel_one(m) + labs(x = "EIR passed to set_equilibrium() (bites per adult per year, log scale)")) |
    dev
  one <- one + plot_annotation(caption = cap(ibm_note, DEV_NOTE, fig_width = 11), theme = theme_cmp()) &
    theme(legend.position = "top", legend.justification = "left")
  save_fig(one, paste0("pv_eir_", m$key), width = 11, height = 4.6)
}
...
## clinical alone, band by band as falciparum's: the evidence for
## age-profile-clinical-pv, which says nothing about prevalence
ap_pv <- age %>% filter(scenario %in% paste0("eir_", PROFILE_EIR_PV)) %>%
  mutate(eir = factor(sprintf("EIR %s", sub("eir_", "", scenario)),
                      levels = sprintf("EIR %g", PROFILE_EIR_PV)))
fw <- 2.0 + 4.0 * length(PROFILE_EIR_PV)
g <- panel_outcome(ap_pv, "clin", "clinical episodes per person-year") +
  plot_annotation(
    title = "P. vivax: clinical incidence by age band",
    caption = cap(BAND_NOTE, fig_width = fw), theme = theme_cmp())
save_fig(g, "pv_age_clin", width = fw, height = 4.6)
...
g <- ggplot(both, aes(y = scenario)) +
  ...
  labs(title = "P. vivax: intervention impact over the first three years, at three transmission levels",
       subtitle = cap(paste("Each intervention deployed unchanged at EIR 1, 3 and 10, relative to",
                            ...
                            "primaquine or tafenoquine. Circle = IBM median with its replicate band",
                            "(median \u00b1 1.28 SD); triangle = fleet, filled solid where it falls",
                            "outside the band."), width = 128),
       x = "reduction relative to baseline", y = NULL,
       caption = cap("Plotted medians are in results/pv/int_impact_summary.csv. Under indoor",
...
if (length(fs)) {
  ...
  g <- ggplot(d, aes(x, y)) +
    ...
    labs(title = sprintf("P. vivax site files: %s sub-site-months across %d countries",
         ...
         caption = cap(paste("Dashed line = perfect agreement. The axes stop at the 99.9th",
